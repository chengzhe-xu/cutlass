...
///////////////////////////////////////////////////////////////////////////////////////////////////
// E.6.1  Configuration constants, Section 6.2 offsets, descriptor and protocol constants
///////////////////////////////////////////////////////////////////////////////////////////////////
...
// Section 6.2 byte offsets from the dynamic shared-memory base
...
// UMMA descriptors (Section 7.6, D.7): hi word constant, lo word = LBO (1 = 16 B) | (start_address >> 4) in a 14-bit field
...
// Protocol constants (Section 7.2)
...
// E.6.1  CuTe cross-check (compile time only): the epilogue smem byte formula uses Swizzle<2,5,2> on byte offsets,
// bits [5,7) ^= bits [7,9) (Layout_MN_SW128_32B_Atom<float>, Section 4; B.3.3 step 17).
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// Trace-only diagnostics (E.7.2 B1/B2, E.7.3): hang records in a host-mapped pinned buffer, heartbeats, K_TAIL
///////////////////////////////////////////////////////////////////////////////////////////////////
#if defined(CUTLASS_DEABSTRACTION_TRACE)
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// E.6.2  PTX wrappers (W1-W37); each asm string is copied from the CUTLASS source named in the comment
///////////////////////////////////////////////////////////////////////////////////////////////////
...
// W1  cutlass::canonical_warp_idx_sync (cutlass.h:127-133)
__device__ __forceinline__ int warp_idx_sync() { return __shfl_sync(0xffffffff, static_cast<int>(threadIdx.x / 32), 0); }

// W2  cute::elect_one_sync (cluster_sm90.hpp:180-197): pred is 1 in exactly one lane of the converged warp, 0 elsewhere
...
// W3/W4  cute::block_rank_in_cluster, cute::block_id_in_cluster (cluster_sm90.hpp:154-158, 126-132)
...
// W5  ClusterBarrier::init (barrier.h:391-403)
...
// W6  ClusterBarrier::wait (barrier.h:410-430): blocking try_wait loop with the suspend-time hint in a register
...
// W7  ClusterBarrier::try_wait (barrier.h:461-478): one peek, no suspend hint; 1 = WaitDone, 0 = WaitAgain
...
// W8  ClusterBarrier::test_wait (barrier.h:435-457): CLC producer_tail only
...
// W9  ClusterBarrier::arrive (local, barrier.h:509-518)
...
// W10  ClusterBarrier::arrive(cta_id, pred) (barrier.h:486-499): remote arrive through mapa; the `if (pred)` stays in C++
...
// W11  cutlass::arch::umma_arrive_2x1SM_sm0 (barrier.h:905-921): arrive on the pair leader's barrier through the peer-bit-masked address
...
// W12  ClusterTransactionBarrier::arrive_and_expect_tx (local, barrier.h:588-598)
...
// W13  ClusterTransactionBarrier::arrive_and_expect_tx(bytes, cta_id, pred) (barrier.h:606-620): every lane executes, lanes with pred == 1 act
...
// W14  cutlass::arch::fence_barrier_init (barrier.h:712-720)
...
// W15  cutlass::arch::fence_view_async_shared (barrier.h:728-736)
...
// W16/W17  cute::cluster_arrive_relaxed, cute::cluster_wait (cluster_sm90.hpp:48-51, 66-69)
...
// W18/W19  NamedBarrier::arrive_and_wait_internal / arrive_internal (barrier.h:285-288, 305-309)
...
// W20  cute::prefetch_tma_descriptor (copy_sm90_desc.hpp:302-317): generic address of the parameter-space descriptor
...
// W21  SM100_TMA_2SM_LOAD_MULTICAST_3D::copy (copy_sm100_tma.hpp:283-305); the barrier operand is already peer-bit-masked by the caller
...
// W22  SM100_TMA_2SM_LOAD_3D::copy (copy_sm100_tma.hpp:104-128)
...
// W23  SM90_TMA_STORE_3D::copy (copy_sm90_tma.hpp:1003-1023)
...
// W24/W25  cute::tma_store_arrive, cute::tma_store_wait<Count> (copy_sm90_tma.hpp:1225-1231, 1248-1256)
...
// W26-W28  cute::TMEM::Allocator2Sm::allocate / free / release_allocation_lock (tmem_allocator_sm100.hpp:134-145, 157-169, 172-179)
...
// W29  SM100_MMA_F16BF16_2x1SM_SS::fma (mma_sm100_umma.hpp:549-587); the elect_one_sync() of the source is at the call site
...
// W30  cutlass::arch::umma_arrive_multicast_2x1SM (barrier.h:848-861); the elect_one_sync() of the source is at the call site
...
// W31  SM100_TMEM_LOAD_32dp32b16x::copy (copy_sm100.hpp:3576-3604): lane l receives 16 consecutive columns of TMEM row (lane) l of the addressed 32-lane group
...
// W32  cutlass::arch::fence_view_async_tmem_load (barrier.h:923-931)
...
// W33  PersistentTileSchedulerSm100::issue_clc_query (sm100_tile_scheduler.hpp:392-405); elected lane of the scheduler warp
...
// W34  PersistentTileSchedulerSm100::work_tile_info_from_clc_response (sm100_tile_scheduler.hpp:409-436); the fence follows in C++.
...
// W35/W36  cutlass::arch::wait_on_dependent_grids / launch_dependent_grids (grid_dependency_control.h:95-98, 85-88);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// E.6.4  CLC consume (every participating warp, once per tile; Section 7.4, B.3.4 steps 5b-5c)
//   wait own full[idx]; decode; fence.proxy.async; remote arrive on CTA 0's empty[idx] (800 per response); ++state;
//   map the cancelled cluster's first-CTA id (x0, y0) to this CTA's tile: (tm, tn) = (2*(y0/2) + x0%2 + x, 2*(x0/2) + y0%2 + y).
//   TraceKind: 0 (no record), K_CLC_SCHED (rank-0 warp 1 lane 0), K_CLC_MMA (warp 0 lane 0); toggle-on only.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// E.6.4  Mainloop producer: warp 2 of every CTA (Section 7.5, B.3.1; CollectiveMma::load, sm100_mma_warpspecialized.hpp:589-629)
///////////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// E.6.4  CLC scheduler: warp 1 of rank 0 (Section 7.4, B.3.4; kernel :681-724)
///////////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// E.6.4  MMA role: warp 0 of every CTA (Section 7.6, B.3.2; kernel :726-805; CollectiveMma::mma, sm100_mma_warpspecialized.hpp:651-712)
///////////////////////////////////////////////////////////////////////////////////////////////////
__device__ __forceinline__ void mma_role(char* smem, uint32_t smem_base, uint32_t x, uint32_t y, uint32_t rank, bool is_leader,
  ...
  uint32_t const T = *reinterpret_cast<uint32_t const*>(smem + OFF_TMEM_BASE_PTR);             // tmem_base_ptr (:731); measured 0, never assumed
#if defined(CUTLASS_DEABSTRACTION_TRACE)
  ...
  do {
    ...
    if (is_leader) {                                                                            // mma() (:762-770)
      ...
      uint32_t const tmem_c = T + 128u * acc_stage;                                             // lane 0, columns [128 st, 128 st + 128)
      int k = k_tiles;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// E.6.4  Epilogue consumers: warps 4-7 of every CTA (Section 7.7, B.3.3; kernel :869-955; CollectiveEpilogue::store, :573-956)
///////////////////////////////////////////////////////////////////////////////////////////////////
__device__ __forceinline__ void epilogue_role(ExplicitGemmParams const& params, char* smem, uint32_t smem_base, uint32_t x, uint32_t y,
  ...
  uint32_t const tmem_lane_off = (32u * w) << 16;                                               // TMEM lanes [32 w, 32 w + 32): row 32 w + l of the CTA tile
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// The kernel (E.6.3): one __global__ function, __launch_bounds__(256, 1), extern __shared__ without alignment attribute,
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// E.5  Host function: the mirror of GemmUniversalAdapter::run() for the explicit kernel
///////////////////////////////////////////////////////////////////////////////////////////////////
#if defined(CUTLASS_DEABSTRACTION_TRACE)
namespace {
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// Toggle-on host entry points called by the harness's guarded blocks (E.7.3): K0 probe kernel, sentinel fill of D (B3),
